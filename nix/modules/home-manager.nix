...
in
{
  options.programs.ax-shell = {
    ...
    configDir = mkOption {
      type = types.str;
      readOnly = true;
      description = "The absolute path to the Ax-Shell configuration directory ($XDG_CONFIG_HOME/ax-shell).";
    };

    currentWallpaperPath = mkOption {
      type = types.str;
      readOnly = true;
      description = "The absolute path to the dynamically updated current wallpaper file.";
    };

    faceIconPath = mkOption {
      type = types.str;
      readOnly = true;
      description = "The absolute path to the managed face icon used by Ax-Shell.";
    };

    hyprlandColorsConfPath = mkOption {
      type = types.str;
      readOnly = true;
      description = "The absolute path to the generated Hyprland colors configuration file.";
    };
    ...
    matugen = {
      settings = mkOption {
        ...
        default = configs.matugenDefault;
      };
  ...
  config = mkIf cfg.enable {
    programs.ax-shell = {
      ...
      currentWallpaperPath = paths.wallpaperFile;
      faceIconPath = paths.faceIconFile;
      hyprlandColorsConfPath = paths.hyprlandColorsConf;
    };
    ...
    programs.ax-shell.hyprlandBinds =
      let
        kb = cfg.settings.keybindings;
      in
      [
        "${kb.restart.prefix}, ${kb.restart.suffix}, exec, ${scripts.reload}/bin/ax-shell-reload"
        "${kb.axmsg.prefix}, ${kb.axmsg.suffix}, exec, notify-send '...'"
        "${kb.dash.prefix}, ${kb.dash.suffix}, exec, ${binaries.axSend} open_dashboard"
        "${kb.bluetooth.prefix}, ${kb.bluetooth.suffix}, exec, ${binaries.axSend} open_bluetooth"
        "${kb.pins.prefix}, ${kb.pins.suffix}, exec, ${binaries.axSend} open_pins"
        "${kb.kanban.prefix}, ${kb.kanban.suffix}, exec, ${binaries.axSend} open_kanban"
        "${kb.launcher.prefix}, ${kb.launcher.suffix}, exec, ${binaries.axSend} open_launcher"
        "${kb.tmux.prefix}, ${kb.tmux.suffix}, exec, ${binaries.axSend} open_tmux"
        "${kb.cliphist.prefix}, ${kb.cliphist.suffix}, exec, ${binaries.axSend} open_cliphist"
        "${kb.toolbox.prefix}, ${kb.toolbox.suffix}, exec, ${binaries.axSend} open_tools"
        "${kb.overview.prefix}, ${kb.overview.suffix}, exec, ${binaries.axSend} open_overview"
        "${kb.wallpapers.prefix}, ${kb.wallpapers.suffix}, exec, ${binaries.axSend} open_wallpapers"
        "${kb.mixer.prefix}, ${kb.mixer.suffix}, exec, ${binaries.axSend} open_mixer"
        "${kb.emoji.prefix}, ${kb.emoji.suffix}, exec, ${binaries.axSend} open_emoji"
        "${kb.power.prefix}, ${kb.power.suffix}, exec, ${binaries.axSend} open_power"
        "${kb.randwall.prefix}, ${kb.randwall.suffix}, exec, ${binaries.axSend} random_wallpaper"
        "${kb.caffeine.prefix}, ${kb.caffeine.suffix}, exec, ${binaries.axSend} toggle_caffeine"
        "${kb.restart_inspector.prefix}, ${kb.restart_inspector.suffix}, exec, GTK_DEBUG=interactive ${scripts.reload}/bin/ax-shell-reload"
      ];

    programs.ax-shell.hyprlandExecOnce =
      if cfg.autostart.enable then
        [
          "swww-daemon"
          "${scripts.themeInit}"
          "${binaries.uwsm} -- ${scripts.runner}/bin/ax-shell-run"
